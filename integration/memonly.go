...
func WrapMemOnly(parent kvdb.Store) *Flushable {
	if parent == nil {
		panic("nil parent")
	}

	return WrapMemOnlyWithDrop(parent, nil)
}

// WrapWithDrop is the same as Wrap, but defines onDrop callback.
func WrapMemOnlyWithDrop(parent kvdb.Store, drop func()) *Flushable {
	if parent == nil {
		panic("nil parent")
	}
...
func (w *Flushable) Load(typ int, name string) {
	w.typ = typ
	w.name = name

	muDiffs.Lock()
	defer muDiffs.Unlock()
	if diffs[w.typ] == nil {
		diffs[w.typ] = map[string]*rbt.Tree{}
	}
	if droppedDBs[w.typ] == nil {
		droppedDBs[w.typ] = map[string]bool{}
	}
	prev := diffs[w.typ][w.name]
	dropped := droppedDBs[w.typ][w.name]
	if prev != nil {
		w.modified = prev
		println("load", w.typ, name, prev.Size())
		diffs[w.typ][w.name] = nil
	}
	if dropped {
		w.underlying = devnulldb.New()
		println("dropped", w.typ, w.name)
	}
}
...
func (w *Flushable) Close() error {
	...
	muDiffs.Lock()
	diffs[w.typ][w.name] = w.modified
	println("save", w.typ, w.name)
	muDiffs.Unlock()
	w.modified = nil
...
func (w *Flushable) Drop() {
	muDiffs.Lock()
	diffs[w.typ][w.name] = nil
	droppedDBs[w.typ][w.name] = true
	println("drop", w.typ, w.name)
	muDiffs.Unlock()
}
