...
func (m *MonotonicContainer) Error() error {
	...
	if len(m.ranges) > 2 {
		for _, r := range m.ranges {
			println(common.Bytes2Hex(r.minKey), common.Bytes2Hex(r.maxKey))
			println(bigendian.BytesToUint64(r.minKey), bigendian.BytesToUint64(r.maxKey))
		}
		println("--")
