...
type PeerLeecher struct {
	...
	totalRequested essteam.Metric
	totalReceived  uint32
	totalProcessed essteam.Metric
...
func (d *PeerLeecher) NotifyChunkReceived(last hash.Event, total essteam.Metric) error {
	op := &receivedChunk{
		last:  last,
		total: total,
	}
...
func (d *PeerLeecher) tryToSync() {
	...
	requestsToSend := make([]essteam.Metric, 0, d.cfg.ParallelChunksDownload)
